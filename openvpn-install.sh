...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	# Automatically use default system resolvers (option 1)
	dns="1"
	echo "Using DNS server: Default system resolvers"
	# Automatically use "first_client" as the client name
	unsanitized_client="first_client"
	# Allow a limited set of characters to avoid conflicts
	client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
	[[ -z "$client" ]] && client="client"
	echo "Using client name: $client"
	echo
